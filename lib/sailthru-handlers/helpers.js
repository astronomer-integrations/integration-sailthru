...
function getProductURL(track, options, originalTrack) {
  ...
  return getValueFromProxy(track, 'properties.url')
    || `${getValueFromProxy(originalTrack, 'context.page.url')}${productId}`
...
function getProductDetails(product, options, originalTrack) {
  ...
  if (details.id) {
    ...
    const vars = getProductCustomProperties(properties);
...
function getUpdatedChart(track, facadeOptions, chart) {
    ...
    return Object.assign(chart, {
      items: chart.items.map((product) => {
        ...
        return Object.assign({}, product, {
          vars: Object.assign({}, product.vars, currentProduct.vars),
          qty: newQty,
        });
